...
function renderBrain(){
 ...
 $("b4").textContent = (MODO==="simple")
   ? (BNLAM<3 ? "sin datos" : (BLAM>0.02 ? "sí" : (BLAM<-0.02 ? "al revés" : "no")))
   : BLAM.toFixed(3);
 ...
 $("b-rows").innerHTML=rows.slice(0,150).map(function(m){
  ...
  return "<tr data-mid='"+m.id+"' style='cursor:pointer'>"+
   ...
   "<td class='mates "+(es.dif>0?"up":"dn")+"'>"+(es.dif>=0?"+":"")+(es.dif*100).toFixed(1)+"</td>"+
...
var GRUPOS=[
 ...
 {id:"g3",t:"Análisis",v:["quant","brain"]},
...
function go(v,sinApilar){
 ...
 VIEW=v;
 ["ini","dash","con","sc","pm","news","quant","brain","sim","cart","inv","lib"].forEach(function(x){$("v-"+x).classList.toggle("on",x===v)});
...
var PAL_VISTAS=[
 ...
 ["quant","Análisis de empresas","momentum y riesgo"],["brain","Oportunidades","arbitraje y señales"],
